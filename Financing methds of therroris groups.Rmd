...
graph_3 <- GTD %>%
  mutate(region_txt = as.character(region_txt)) %>%
  count(region_txt, iyear) %>%
  group_by(region_txt) %>%
  mutate(percent = n / sum(n)) %>%
  filter(n > 100) %>%
  ungroup %>%
  ggplot(aes(iyear, percent, color = region_txt)) +
  geom_line() +
  geom_point() +
  scale_y_continuous(labels = percent_format()) +
  ggtitle("Graph 1: By year") +
...
summary_words <- GTD %>%
  ...
  mutate(summary = str_replace_all(summary, number.pattern , ""))  %>%
...
```{r EXTRACT SUMMARIES, echo=FALSE, include=FALSE}
 grep("attacked", GTD$summary, value = TRUE) %>% head(100)

GTD %>%
  filter(attacktype1 == 6) %>% summarise(summary)
...
stealing <- c("bank", "robbery", "theft", "break-in", "burglary", "heist", "looting", "larceny")
extortion <- c("extortion", "shakedown", "money", "payment")
kidnap <- c("kidnap", "kidnapping", "capture", "hijack", "abducted")
all_words <- c("bank", "robbery", "theft", "break-in", "burglary", "heist", "looting", "larceny", "extortion", "shakedown", "money", "payment",
               "kidnap", "kidnapping", "capture", "hijack", "abducted", "abduction")

# Odds ration stelaing

x <- control_or %>% filter(word %in% all_words) %>%
  filter( (Territorial_con + No_territroail_con) > 30) %>%
  arrange(desc(or))
  
  
y <- control_or %>% filter(word %in% extortion ) %>%
  filter( (Territorial_con + No_territroail_con) > 10) %>%
  arrange(desc(or))
    
z <- control_or %>% filter(word %in% kidnap ) %>%
  filter( (Territorial_con + No_territroail_con) > 10) %>%
  arrange(desc(or))
...
# Preparing data
# Robbery
t <- grep("bank", GTD$summary, value = FALSE) 
GTD$robbery <- 0
GTD$robbery[t] <- 1
t <- grep("robbery", GTD$summary, value = FALSE) 
GTD$robbery[t] <- 1
